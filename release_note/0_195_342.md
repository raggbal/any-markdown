...
新規Markdownファイルを作成してfractalで開いた際、エディタが空であるにもかかわらず「Start typing...」のプレースホルダーテキストが表示されない。カーソルの入力位置が分かりづらい。

## 原因

CSSセレクタ `.editor:empty:before` を使用していたが、`:empty` 擬似クラスはDOM上に子要素が一切ない場合にのみマッチする。fractalでは空のMarkdownファイルでも `<p><br></p>` を必ず挿入するため（カーソル配置用）、`:empty` 条件が満たされずプレースホルダーが表示されなかった。
