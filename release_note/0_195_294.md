...
| --- | --- | --- | --- | --- | --- |
| Enter前 | `a<br>b` | 1 | 0 | 2 | 2（正常） |
| Enter後（バグあり） | `a<br><br>b` | 2 | **-1（誤）** | **2** | **3**（1行足りない） |
| Enter後（修正後） | `a<br><br>b` | 2 | 0 | 3 | 3（正常） |
...
`getCurrentLineInBlock` と同じパターンで、`lineBreaks` 配列から sentinel `<br>` を除外する処理を WeakSet ベースからDOM動的検査に変更。
...
| DOMパターン | sentinel? | 説明 |
| --- | --- | --- |
| `text<br>` | No | 通常の行末。最後の `<br>` の前がテキストノード |
| `text<br><br>` | Yes | 末尾空行。最後の `<br>` の前が `<br>` |
| `text<br>[empty text]<br>` | Yes | 末尾空行の変形。最後の `<br>` の前が空テキストノード |
| `[data-trailing-br="true"]...<br>` | Yes | 表示モードで追加された trailing BR |
