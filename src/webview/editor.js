(function() {
    ...
    function setCursorToLastLineStart(element) {
        ...
        if (lastBreak.type === 'br') {
            ...
            while (sib) {
                if (sib.nodeType === 3 && sib.textContent.length > 0 && sib.textContent !== '\n' && sib.textContent.trim() !== '') {
        ...
        if (hasContentAfterLastBreak) {
            ...
            // We need to position at the start of this empty last line
            // That means: position right after the last line break
            if (lastBreak.type === 'br') {
                var parent = lastBreak.node.parentNode;
                var brIndex = Array.from(parent.childNodes).indexOf(lastBreak.node);
                range.setStart(parent, brIndex + 1);
                logger.log('setCursorToLastLineStart: positioned after last BR (empty last line)');
            } else {
                // \n at end - position right after the \n
                range.setStart(lastBreak.node, lastBreak.offset + 1);
                logger.log('setCursorToLastLineStart: positioned after last \\n (empty last line)');
            }
        }
