(function() {
    ...
    const i18n = __I18N__;
    console.log('[Any MD] i18n loaded:', i18n.livePreviewMode ? 'OK' : 'EMPTY', '- Sample:', i18n.bold || '(none)');
    ...
    let markdown = decodeURIComponent(escape(atob(__CONTENT__)));
    ...
    function scrollCursorIntoView() {
        ...
        try {
            ...
            // Create a temporary span to get cursor position
            const tempSpan = document.createElement('span');
            tempSpan.style.cssText = 'position:absolute;width:1px;height:1em;';
            range.insertNode(tempSpan);
            
            // Scroll the span into view
            tempSpan.scrollIntoView({ block: 'nearest', behavior: 'instant' });
            
            // Remove the temporary span
            tempSpan.parentNode.removeChild(tempSpan);
        } catch (e) {
            // Fallback: do nothing if scroll fails
    ...
    function setCursorToLastLineStart(element) {
        ...
        logger.log('setCursorToLastLineStart: brElements.length =', brElements.length);
        
        if (brElements.length === 0) {
            // No BR, single line - go to start
            setCursorToStart(element);
            return;
        }
        
        // Line structure in a cell:
        // Line 0: content before first BR
        // Line 1: content after first BR (brElements[0]), before second BR
        // ...
        // Line N: content after BR[N-1] (brElements[N-1])
        //
        // If the last line has text content (text node after last BR), 
        // position at the start of that text node.
        // If the last line is empty (last BR is the last child, or followed only by BR),
        ...
        if (hasTextAfterLastBr) {
            ...
            if (brElements.length >= 2) {
                ...
                if (nodeAfterSecondToLastBr && nodeAfterSecondToLastBr.nodeType === 3 && nodeAfterSecondToLastBr.textContent.length > 0) {
                    ...
                    logger.log('setCursorToLastLineStart: positioned after second-to-last BR at index', brIndex + 1);
                }
            } else {
                // Only 1 BR - position after it (start of line 1, which is empty)
                const parent = lastBr.parentNode;
                const brIndex = Array.from(parent.childNodes).indexOf(lastBr);
                range.setStart(parent, brIndex + 1);
                logger.log('setCursorToLastLineStart: positioned after only BR at index', brIndex + 1);
            }
        }
            ...
            if (fenceMatch) {
                ...
                if (inCodeBlock) {
                    ...
                    if (fenceCharacter === codeFenceChar && fenceLen >= codeFenceLength && afterFence.trim() === '') {
                        // Valid closing fence
                        // Keep the code content as-is, preserving trailing empty lines
                        // Only remove the very last newline that was added by the line processing loop
                        // (each line adds '\n', so the last line also has an extra '\n')
    ...
    function checkBlockPatterns(trigger) {
        ...
        // Convert list item to task list: [ ] + space at the beginning of a list item
        // This handles the case where user types "- " first (creates ul), then "[ ] " to convert to task
        if (liNode && liNode.tagName && liNode.tagName.toUpperCase() === 'LI' && trigger === 'space') {
            ...
            if (taskInListMatch) {
                ...
                checkbox.checked = taskInListMatch[1].toLowerCase() === 'x';
            ...
            if (currentElement && currentElement !== editor) {
                ...
                if (e.key === 'ArrowUp') {
                    ...
                    if (prev) {
                        ...
                        if (tag === 'table') {
                            // Enter table from below - go to last row, first column
                            e.preventDefault();
                            const rows = prev.querySelectorAll('tr');
                            if (rows.length > 0) {
                                const lastRow = rows[rows.length - 1];
                                const firstCell = lastRow.cells[0]; // First column, not last
                                if (firstCell) {
                                    activeTable = prev;
                                    activeTableCell = firstCell;
                                    setCursorToEnd(firstCell);
                    ...
                    if (next) {
                        ...
                        if (tag === 'table') {
                            // Enter table from above - go to first row, first column
                            e.preventDefault();
            ...
            if (range.startOffset === 0 && range.collapsed) {
                ...
                if (tag === 'blockquote') {
                    ...
                    if (textBeforeCursor.length === 0) {
                        // Truly at the beginning - convert to paragraph
                        e.preventDefault();
                        const p = document.createElement('p');
                        p.innerHTML = currentLine.innerHTML || '<br>';
                        currentLine.replaceWith(p);
    ...
    editor.addEventListener('paste', function(e) {
        ...
        if (internalMd) {
            ...
            try {
                ...
                // Custom rule: Ignore presentation tables (layout tables) and process their content
                turndownService.addRule('presentationTable', {
                    filter: function(node) {
                        return node.nodeName === 'TABLE' && 
                               node.getAttribute('role') === 'presentation';
                    },
                    replacement: function(content, node, options) {
                        // Return the content inside the presentation table without table formatting
                        return content;
                    }
                });
                ...
                // Custom rule: Handle complex tables with block content in cells
                // These tables cannot be properly represented as Markdown tables
                turndownService.addRule('complexTable', {
                    filter: function(node) {
                        if (node.nodeName !== 'TABLE') return false;
                        // Skip presentation tables (handled by another rule)
                        if (node.getAttribute('role') === 'presentation') return false;
                        // Check if any cell contains block elements (p, div, ul, ol, etc.)
                        const cells = node.querySelectorAll('td, th');
                        for (let i = 0; i < cells.length; i++) {
                            const cell = cells[i];
                            const blockElements = cell.querySelectorAll('p, div, ul, ol, blockquote, pre, h1, h2, h3, h4, h5, h6');
                            if (blockElements.length > 1) {
                                return true; // Complex table with multiple block elements in a cell
                            }
                        }
                        return false;
                    },
                    replacement: function(content, node, options) {
                        // Convert complex table to a series of sections
                        let result = '';
                        const rows = node.querySelectorAll('tr');
                        rows.forEach(function(row) {
                            const cells = row.querySelectorAll('td, th');
                            if (cells.length >= 2) {
                                // Two-column layout: first column as header, second as content
                                ...
                                // Process content cell - let Turndown handle the inner content
                                const tempDiv = document.createElement('div');
                                tempDiv.innerHTML = contentCell.innerHTML;
                                const cellContent = turndownService.turndown(tempDiv.innerHTML);
                                result += cellContent;
                            } else if (cells.length === 1) {
                                // Single column - just process content
                                const tempDiv = document.createElement('div');
                                tempDiv.innerHTML = cells[0].innerHTML;
                                const cellContent = turndownService.turndown(tempDiv.innerHTML);
                                result += '\n\n' + cellContent;
                            }
                        });
